...
MODEL_NAME = os.getenv("MODEL_NAME", "compression")
ADAPTER_PATH = ADAPTERS.get(MODEL_NAME)
model_safe = os.getenv("MODEL_NAME", "default")
...
if ADAPTER_PATH is None:
    raise ValueError(f"No adapter mapped for: {MODEL_NAME}")
...
def get_nlp():
    ...
    if _nlp is None:
        try:
            ...
            log(f"[WARN] spaCy model not available: {e}")
            _nlp = None
    return _nlp
...
def _parse_speed_cell(s: str, imperial: bool) -> str:
    ...
    if "-" in num:
        ...
        try:
            ...
            return "-"
        if imperial:
            a = _mph_to_kmh_value(a); b = _mph_to_kmh_value(b)
...
def parse_response(html_content):
    ...
    log(f"[PARSE TEMP ROW] {table.find('tr','temperatures').get_text(' | ', strip=True)}")
    log(f"[PARSE FEEL ROW] {table.find('tr','windchills').get_text(' | ', strip=True)}")
...
log(f"Loading model: {MODEL_NAME} from {MODEL_PATH}")
try:
                ...
                if adapter == "meteo":
                    ...
                    if lat and lon:
                        ...
                        if not data:
                            ...
                            resp_text = sanitize_output(
                                out.split("<|start_header_id|>assistant<|end_header_id|>")[-1],
                                adapter,
                            )
                            split_text = out.split("<|start_header_id|>assistant<|end_header_id|>")[-1]
                            log(f"[GEN RAW {adapter}] {out!r}")
                            log(f"[GEN SPLIT {adapter}] {split_text!r}")
                            log(f"[GEN CLEAN {adapter}] {resp_text!r}")
                    else:
                        resp_text = "No valid city found in the input."
                elif adapter == "durangaldea":
                    output = run_durangaldea_model(text)
                    resp_text = output or "[EMPTY RESPONSE]"
                else:
